...
describe('computeFieldSprites', () => {
  ...
  it('does not fuse the field floor into bottom-row connections', () => {
    // Bottom-row puyos keep the standard rounded silhouette — the
    // field frame is not treated as a same-color neighbour, so a lone
    // puyo on y=0 reports `down: false`.
    const match = createMatchState({ seed: 1, colorMode: 4, players: [{ id: 'A' }] });
    const p = match.players[0]!;
    setCell(p.board, 0, 0, 'R');
    const sprite = computeFieldSprites(p).find((s) => s.id === 'cell:0:0')!;
    expect(sprite.connections?.down).toBe(false);
  });
